...
class Embed(nn.Module):
    def __init__(self, input_dim, dims, normalize_input=True, activation='gelu'):
        ...
        if len(dims) > 0:
            # 中间宽度随输出维度缩放: 64→32(原样), 128→64(L版)
            hidden_dim = dims[-1] // 2
            
            # Stage 1: Depthwise expansion
            ...
            # Stage 3: Pointwise expansion to output
            self.pw_conv2 = nn.Conv1d(hidden_dim, dims[-1], kernel_size=1)
            self.pw_bn2 = nn.BatchNorm1d(dims[-1])
...
class Block(nn.Module):
    def __init__(self, embed_dim=128, num_heads=8, num_MIheads=64,
...
class BlockMI(nn.Module):
    def __init__(self, embed_dim=128, num_MIheads=64, num_heads=8,
                 dropout=0.1, attn_dropout=0.1, activation_dropout=0.1,
                 add_bias_kv=False, activation='gelu',
                 scale_fc=True, scale_attn=True, scale_heads=True, scale_resids=True,
                 hourglass_dim=None, num_subblocks=2):
...
class MIParticleTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        hourglass_dim = embed_dim // 2 if hourglass_dim is None else hourglass_dim
        default_cfg = dict(embed_dim=embed_dim, num_heads=num_heads, num_MIheads=pair_embed_dims[-1],
                           dropout=0.1, attn_dropout=0.1, activation_dropout=0.1,
...
def get_model(data_config, **kwargs):
    ...
    cfg = dict(
        ...
        hourglass_dim=int(embed_dim * 0.5),   # d_h = 32
        num_subblocks=2,                       # K
        # misc
        trim=True,
        for_inference=False,
